      ...
      if (($this->pages) > 1) {
        ...
        if ($this->page != 1) {
          if ($this->_showFirstAndLast) {
            $plinks[] = ' <a href="?page=1'.$queryUrl.'">&laquo;&laquo;Premier</a> ';
          }
          $plinks[] = ' <a href="?page='.($this->page - 1).$queryUrl.'">&laquo;Précédent</a>&nbsp;&nbsp; ';
        }
        
        // Assign all the page numbers & links to the array
        for ($m = 1; $m < ($this->pages + 1); $m++) {
          if ($this->page == $m) {
            $links[] = ' <a class="selected">'.$m.'</a> '; // If we are on the same page as the current item
          } else {
            $links[] = ' <a href="?page='.$m.$queryUrl.'">'.$m.'</a> '; // add the link to the array
          }
        }

        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' &nbsp;&nbsp;<a href="?page='.($this->page + 1).$queryUrl.'">Suivant&raquo;</a> ';
          if ($this->_showFirstAndLast) {
            $slinks[] = ' <a href="?page='.($this->pages).$queryUrl.'">Dernier&raquo;&raquo;</a> ';
          }
        }
